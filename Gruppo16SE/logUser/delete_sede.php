...
if (isset($_POST['id_sede'])){
	...
	$sql = ("DELETE FROM sede WHERE id_sede=$1");
	$result = pg_prepare($db, "delete_sede", $sql);
	$ret = pg_execute($db, "delete_sede", array($_POST['id_sede']));
...
else { echo 'Errore nella cancellazione di questa sede';}